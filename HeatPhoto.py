...
def get_params():
    params = {
        'weight': r'D:\Desktop\yolo\Run\QAQ_11s_48batch\weights\best.pt',  # 训练出来的权重文件
        'cfg': r'yolo11s.yaml',  # 训练权重对应的yaml配置文件
        'device': 'cuda:0',
        'method': 'XGradCAM',  # GradCAMPlusPlus, GradCAM, XGradCAM , 使用的热力图库文件不同的效果不一样可以多尝试
        'layer': 'model.model[16]',  # 想要检测的对应层
        'backward_type': 'all',  # class, box, all
        'conf_threshold': 0.5,  # 0.6  # 置信度阈值，有的时候你的进度条到一半就停止了就是因为没有高于此值的了
        'ratio': 0.02  # 0.02-0.1
    }
    return params
...
if __name__ == '__main__':
    ...
    model(r'D:\Desktop\yolo\YOLOv11\Dataset\Photo\DJI_0717.JPG', r'D:\Desktop\yolo\hotResult')  # 第一个是检测的文件, 第二个是保存的路径